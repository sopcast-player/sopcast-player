...
class DatabaseOperations(object):
    def __init__(self):
        ...
        if create_tables == True:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'CREATE TABLE bookmarks (id INTEGER PRIMARY KEY, channel_name VARCHAR(100), channel_url VARCHAR(255))')
            cursor.execute(
                'CREATE TABLE channel_groups (id INTEGER PRIMARY KEY, type INT, en_name VARCHAR(255), cn_name VARCHAR(255), ' \
                'name VARCHAR(255), description VARCHAR(1024))')
            cursor.execute(
                'CREATE TABLE channels (id INTEGER PRIMARY KEY, type INTEGER, btype INTEGER, language VARCHAR(255), ' \
    ...
    def retrieve_channels_by_channel_group_id(self, channel_group_id):
        ...
        cursor.execute(
            "SELECT id, en_name, description, region, class_en, stream_type, kbps, qs, qc, sop_address FROM channels WHERE channel_group_id=?",
            row)
        data = cursor.fetchall()
        conn.close()
        return data
    ...
    def retrieve_channels_by_channel_group_id_cn(self, channel_group_id):
        ...
        row = [channel_group_id]
        cursor.execute(
            "SELECT id, cn_name, cn_description, region_cn, class_cn, stream_type, kbps, qs, qc, sop_address FROM channels WHERE channel_group_id=?",
            row)
        data = cursor.fetchall()
        conn.close()
        return data
